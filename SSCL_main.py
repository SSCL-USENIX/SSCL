...
if __name__ == "__main__":
    ...
    parser.add_argument('--b_m', type=float, default=0.5, metavar='S',help='batch memory ratio(default: 0.2)')
    ...
    parser.add_argument('--bma', type=float, metavar='S',default=0.5,help='batch minority allocation(default: 0)')
    ...
    parser.add_argument('--upper_thresh', type=float, metavar='S',default=0.1,help='Sets the upper threshold for while aligning similarity (default:0.2)')
    parser.add_argument('--temp', type=float, metavar='S',default=10,help='temperature (default:10)')
    parser.add_argument('--temp2', type=float, metavar='S',default=3,help='temperature2 (default:1.5)')
    ...
    for seed_value in seed_list:
        ...
        proc = subprocess.Popen(["python SSCL.py --seed="+str(seed_value)+" --ds="+str(args.ds)+" --lr="+str(args.lr)+" --upper_thresh="+str(args.upper_thresh)+" --analyst_labels="+str(args.analyst_labels)+" --temp="+str(args.temp)+" --temp2="+str(args.temp2)+" --wd="+str(args.wd)+" --alpha="+str(args.alpha)+" --gpu="+str(args.gpu)+" --filename="+str(temp_file_name)+" --cos_dist="+str(args.cos_dist)+" --mode_val="+str(args.mode_val)+" --bool_gpm="+str(args.bool_gpm)+" --mem_strat="+str(args.mem_strat)+" --b_m="+str(args.b_m)+" --label_ratio="+str(args.label_ratio)+" --lab_samp_in_mem_ratio="+str(args.lab_samp_in_mem_ratio)+" --nps="+str(args.nps)+" --bma="+str(args.bma)+" --training_cutoff="+str(args.training_cutoff)+" --bool_closs="+str(args.bool_closs)+" --mlps="+str(args.mlps)],shell=True,cwd=curr_dir)
        proc.communicate()
    ...
    for key, value in auc_results.items():
        ...
        aut_results[key].extend([float(value[7]),float(value[8]),float(value[9]),float(value[10])])
    ...
    print("-"*80)
    print("{:<20}  {:<20}  {:<20}".format('Benchmark Mean',float(str(aut_average[12])[:5]), float(str(aut_average[13])[:5])))
    print("{:<20}  {:<20}  {:<20}".format('Benchmark Variance',float(str(auc_std[12])[:5]), float(str(auc_std[13])[:5])))
    print("-"*80)
    ...
    print("-"*80)
    print("{:<20}  {:<20}  {:<20}  {:<20}  {:<20}  {:<20}  {:<20}".format('Cols','Self_labels (Benign)', 'Self_labels (Attack)','Total (self-label)','Analyst (Benign)', 'Analyst (Attack)','Total (analyst-label)'))
    print("-"*80)
    ...
    print("-"*80)
    print("{:<20}  {:<2}  {:<2} {:<2}".format('Avg no of rejected samples (seen task)',float(str(aut_average[10])[:5]), '±',float(str(auc_std[10])[:5])))
    print("{:<20}  {:<2}  {:<2} {:<2}".format('Avg np of rejected samples(unseen task)',float(str(aut_average[11])[:5]), '±',float(str(auc_std[11])[:5])))
    print("-"*80)
